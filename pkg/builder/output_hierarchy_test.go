...
func TestOutputHierarchyCreation(t *testing.T) {
	t.Run("AbsoluteWorkingDirectory", func(t *testing.T) {
		...
		testutil.RequireEqualStatus(t, err, status.Error(codes.InvalidArgument, "Invalid working directory: Path is absolute, while a relative path was expected"))
	})
	...
	t.Run("InvalidWorkingDirectory", func(t *testing.T) {
		...
		testutil.RequireEqualStatus(t, err, status.Error(codes.InvalidArgument, "Invalid working directory: Path resolves to a location outside the input root directory"))
	})
	...
	t.Run("AbsoluteOutputDirectory", func(t *testing.T) {
		...
		testutil.RequireEqualStatus(t, err, status.Error(codes.InvalidArgument, "Invalid output directory \"/etc/passwd\": Path is absolute, while a relative path was expected"))
	})
	...
	t.Run("InvalidOutputDirectory", func(t *testing.T) {
		...
		testutil.RequireEqualStatus(t, err, status.Error(codes.InvalidArgument, "Invalid output directory \"../..\": Path resolves to a location outside the input root directory"))
	})
	...
	t.Run("InvalidOutputFile", func(t *testing.T) {
		...
		testutil.RequireEqualStatus(t, err, status.Error(codes.InvalidArgument, "Output file \"..\" resolves to the input root directory"))
	})
}
